...
function sendRequestProductsPatch(request, bodyData, headersData, paramsData) {
    return request.patch(`${process.env.URL}/products/${paramsData}`, {
      data: bodyData,
      headers: headersData
    });
}
...
export default function createProductChangeInfoTests() { 
    test("PATCH product in store with valid data as seller @happy", async ({ request }) => {
        ...
        const response = await sendRequestProductsPatch(request,bodyData,headersData,buf.product_id);

        expect(response.status()).toBe(200);

        const responseGet = await sendRequestProductsGetFullInfo(request, buf.product_id);
        const resBodyGet = JSON.parse(await responseGet.text()); 
        expect(resBodyGet).toBe(bodyData);
    });

    test("PATCH product in store with valid data, but not as seller @happy", async ({ request }) => {
        const bodyData = createValidBodyData();
    ...
    test("PATCH product in store with valid data, but not authorizated@happy", async ({ request }) => {
        const bodyData = createValidBodyData();
        
        var buf = readFromReusableDataFile();
        const response = await sendRequestProductsPatch(request,bodyData,{},buf.product_id);
      
        expect(response.status()).toBe(401);
    });

    test("PATCH product in store with invalid data as seller @happy", async ({ request }) => {
